...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.rectangle(np.asarray(img), c1, c2, color, -1)  # filled
        cv2.putText(np.asarray(img), label, (c1[0], c1[1] - 2), 0, tl / 5, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        #cv2.circle( np.asarray(img), center, radius, color, 3 )
        #font = cv2.FONT_HERSHEY_COMPLEX
        #cv2.putText( img, label, (center[ 0 ] - 30, center[ 1 ] + 10),font, 1, color, 2 )
